...
@objc public class CapacitorSQLite: NSObject {
                ...
                if let isBioAuth = config.biometricAuth {
                                if isBioAuth == 1 {
                                    ...
                                    do {
                                        ...
                                        initMessage = "Biometric authentication check failed :"
                                        initMessage.append(" \(error.localizedDescription)")
                                    }
                                }
                            }
   ...
   // MARK: - ValidateEncryptionSecret
        ...
        if isInit {
            ...
            do {
                ...
                let mDb: Database = try Database(
                    ...
                    mode: "no-encryption", version: version, key: "", readonly: true,
                    vUpgDict: [:])
                dbDict[connName] = mDb
                return
            } catch let error {
    ...
    @objc public func createConnection(_ dbName: String,
                                       ...
                                       version: Int,
                                       key: String,
                                       vUpgDict: [Int: [String: Any]], readonly: Bool) throws {
        if isInit {
            ...
            do {
                let mDb: Database = try Database(
                    ...
                    mode: mode, version: version, key: key, readonly: readonly,
                    vUpgDict: vUpgDict)
            ...
            if let data = ("["+parsingData+"]").data(using: .utf8) {
                ...
                do {
                    mDb = try Database(
                        ...
                        encrypted: encrypted, isEncryption: isEncryption, account: account,
                        mode: inMode, version: version, key: "", readonly: false, vUpgDict: [:])
